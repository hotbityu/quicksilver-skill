...
- `references/unit-init.md`: focused QS base metadata index.
- `references/unit-system-tables.md`: common QS system table fields for dictionaries, menus, privileges, roles, and text resources.
...
- For an ordinary-text delete target, first search existing SQL under the selected module project's `src/main/resources/QS-MODULE/data/sql/default` tree for `insert into TsTextResource` statements with matching `FValue`.
